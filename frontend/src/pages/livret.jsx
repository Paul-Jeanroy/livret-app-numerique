...
/* Page livret: permet l'affichage selon le rôle du livret à completer.
    ...
    - sp_check_periode_completion : Vérifie si la période de formation est déjà complète.
    - sp_changer_evaluation : Gère le changement d'évaluation dans le tableau d'évaluation.
    - sp_generer_navigation : Génère la navigation entre les périodes de formation.
    - sp_valider_livret : Enregistre les évaluations dans la base de données. 
...
export default function Livret() {
    ...
    const [f_periode_complete, setPeriodeComplete] = useState(false);
    const [i_livret_id, setLivretId] = useState(null);
    ...
    useEffect(() => {
        const sp_get_apprentis = async () => {
            try {
                ...
                console.error("Erreur lors de la récupération des/de l'apprenti(s):", error);
            }
        };
    ...
    useEffect(() => {
        const sp_check_periode_completion = async () => {
            ...
            try {
                ...
                setPeriodeComplete(response.data.completed);
                if (response.data.completed) {
                    ...
                    setLivretId(response.data.completed.id_livret);
                } else {
    ...
    const sp_generer_navigation = (periode) => {
        // Période Trimestre
        // w_tt_annees L3,M1...
        const navButtons = [];
        const periodeNames = {
        ...
        if (!w_tt_annees || !w_tt_annees.length) {
            return navButtons;
        }

        for (let i = 0; i < w_tt_annees.length; i++) {
            periodeNames[periodeKey].forEach((p, pIndex) => {
                const periodLabel = `${w_tt_annees[i].annee}-${pIndex + 1} - ${p}`;
                navButtons.push(
                    <button
                        key={`${w_tt_annees[i].annee}-${pIndex}`}
    ...
    const sp_valider_livret = async () => {
        ...
        try {
            ...
            const url = i_livret_id
                ? `http://localhost:5000/livret/setUpdateEvaluation/${i_livret_id}`
                : "http://localhost:5000/livret/setEvaluation";
            const response = i_livret_id
                ? await axios.put(url, o_livret)
                : await axios.post(url, o_livret);
    ...
    const navigationButtons = sp_generer_navigation(formationData?.formation?.periode, formationData?.duree);
            ...
            <main className="main-container-livret">
                ...
                {!f_fiche_signaletique ? (
                    <div className="div-container-livret">
                        ...
                        <img className="fleche-gauche-livret" src="icon-arrow.svg" style={{ transform: "rotate(180deg)" }} onClick={() => sp_naviguer_periode(-1)} />
                        <img className="fleche-droite-livret" src="icon-arrow.svg" onClick={() => sp_naviguer_periode(1)} />
                                ...
                                {formationData?.blocs &&
                                    w_tt_evaluations.map((bloc, index) => (
                                        <GrilleEvaluationLivret
                                            key={index}
                                            bloc={bloc}
                                            onChange={(updatedCompetences) => sp_changer_evaluation(index, updatedCompetences)}
                                            completed={f_periode_complete}
                                        />
                                    ))}
                                <div className="btn-validation-livret">
                                    <button onClick={sp_valider_livret}>Valider le livret</button>
                                </div>
                            </>
                        )}
